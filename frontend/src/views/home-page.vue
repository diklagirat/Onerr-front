<template>
    <section class="home-page">
        <div class="main-content">
            <div class="hero">
                <div class="hero-container">
                    <h1>Find the perfect <i>freelance</i> services for your business</h1>
                    <input type="search" placeholder="Try &quot;building mobile app" />
                </div>
            </div>

            <div class="trusted-by-container">
                <span class="trusted-by-title">Trusted by:</span>
                <ul class="trusted-by-list">
                    <li>
                        <picture> Facebook</picture>
                    </li>
                    <li>
                        <picture>Google</picture>
                    </li>
                    <li>
                        <picture>Netflix</picture>
                    </li>
                    <li>
                        <picture>P&G</picture>
                    </li>
                    <li>
                        <picture>PayPal</picture>
                    </li>
                </ul>
            </div>

            <div>Popular professional services List</div>

            <div>Freelance Info
                <h2>A whole world of freelance talent at your fingertips</h2>
                <ul>
                    <li>
                        <h6>
                            <span>icon</span>The best for every budget
                        </h6>
                        <p>Find high-quality services at every price point. No hourly rates, just project-based pricing.
                        </p>
                    </li>
                    <li>
                        <h6>
                            <span>icon</span>Quality work done quickly
                        </h6>
                        <p>Find the right freelancer to begin working on your project within minutes.
                        </p>
                    </li>
                    <li>
                        <h6>
                            <span>icon</span>Protected payments, every time
                        </h6>
                        <p>Always know what you'll pay upfront. Your payment isn't released until you approve the work.
                        </p>
                    </li>
                    <li>
                        <h6>
                            <span>icon</span>24/7 support
                        </h6>
                        <p>Questions? Our round-the-clock support team is available to help anytime, anywhere.
                        </p>
                    </li>
                </ul>
            </div>

            <div class="signup-image">
                <h2>Find the <i>talent</i> needed to get your business <i>growing.</i></h2>
                <a href="#">Get Started</a>
            </div>

        </div>
    </section>
</template>
 
<script>
export default {

}
</script>
